...
export class ActivityPrismaService
  ...
  constructor() {
    ...
    const host = process.env.LOGGING_DB_HOST ?? '127.0.0.1';
    const port = Number(process.env.LOGGING_DB_PORT ?? 3306);
    const user = process.env.LOGGING_DB_USER ?? 'root';
    const password = process.env.LOGGING_DB_PASS ?? undefined; // undefined if none
